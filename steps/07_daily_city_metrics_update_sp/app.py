...
def create_daily_city_metrics_table(session):
    ...
    dcm = (
        session.create_dataframe(
            [[None] * len(DAILY_CITY_METRICS_SCHEMA.names)],
            schema=DAILY_CITY_METRICS_SCHEMA,
        )
        .na.drop()
        .write.mode("overwrite")
        .save_as_table("ANALYTICS.DAILY_CITY_METRICS")
    )
...
def merge_daily_city_metrics(session):
    ...
    print(
        "{} records in stream".format(session.table("HARMONIZED.ORDERS_STREAM").count())
    )
    orders_stream_dates = (
        session.table("HARMONIZED.ORDERS_STREAM")
    ...
    weather = weather.join(
        weather_pc,
        (weather["POSTAL_CODE"] == weather_pc["POSTAL_CODE"])
        & (weather["COUNTRY"] == weather_pc["COUNTRY"]),
        rsuffix="_pc",
    )
    weather = weather.join(
        countries,
        (weather["COUNTRY"] == countries["ISO_COUNTRY"])
        & (weather["CITY_NAME"] == countries["CITY"]),
        rsuffix="_c",
    )
    weather = weather.join(
        orders_stream_dates, weather["DATE_VALID_STD"] == orders_stream_dates["DATE"]
    )

    weather_agg = (
        weather.group_by(
            F.col("DATE_VALID_STD"), F.col("CITY_NAME"), F.col("COUNTRY_C")
        )
        .agg(
            F.avg("AVG_TEMPERATURE_AIR_2M_F").alias("AVG_TEMPERATURE_F"),
            F.avg(
                F.call_udf(
                    "ANALYTICS.FAHRENHEIT_TO_CELSIUS_UDF",
            ...
            F.avg(
                F.call_udf(
                    "ANALYTICS.INCH_TO_MILLIMETER_UDF", F.col("TOT_PRECIPITATION_IN")
                )
            ).alias("AVG_PRECIPITATION_MM"),
        ...
        .select(
            ...
            F.round(F.col("AVG_PRECIPITATION_MM"), 2).alias(
    ...
    daily_city_metrics_stg = orders.join(
        weather_agg,
        (orders["DATE"] == weather_agg["DATE"])
        & (orders["CITY_NAME"] == weather_agg["CITY_NAME"])
        & (orders["COUNTRY_DESC"] == weather_agg["COUNTRY_DESC"]),
        how="left",
        rsuffix="_w",
    ).select(
        "DATE",
    ...
    cols_to_update = {
        c: daily_city_metrics_stg[c] for c in daily_city_metrics_stg.schema.names
    }
    ...
    dcm.merge(
        ...
        (dcm["DATE"] == daily_city_metrics_stg["DATE"])
        & (dcm["CITY_NAME"] == daily_city_metrics_stg["CITY_NAME"])
        & (dcm["COUNTRY_DESC"] == daily_city_metrics_stg["COUNTRY_DESC"]),
